...
-- 12) groups 테이블 (v1.3 신규). categories 와 동일 컬럼 구성 (icon 없음).
...
-- 13) groups RLS — 본인 row 만 (categories 와 동일 패턴)
...
-- 15) groups Realtime publication
do $$
begin
  if not exists (
    select 1 from pg_publication_tables
...
-- 16) categories.group_id 컬럼 (v1.3 신규). null = '미분류'. FK 제약 두지 않음
