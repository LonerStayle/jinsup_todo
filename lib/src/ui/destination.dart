...
enum DestinationKind { today, category, outline, timeline }
...
/// - `timeline` (kind=timeline) 이 세 번째 — 단축키 2. (날짜 지정된 항목 전역 타임라인)
...
class AppDestination {
  ...
  bool get isTimeline => kind == DestinationKind.timeline;
  ...
  ///
  /// 순서·단축키: today (digit 0) → outline (digit 1) → timeline (digit 2) →
  ...
  static List<AppDestination> buildAll(List<Category> categories) {
    final dests = <AppDestination>[
      ...
      // v1.5 — 날짜 지정된 항목 전역 타임라인. 단축키 2.
      const AppDestination._(
        kind: DestinationKind.timeline,
        label: '타임라인',
